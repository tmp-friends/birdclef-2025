...
def make_collate_fn(augmentor):
    ...
    def _collate(batch):
        # augmentor.cutmix で同バッチ画像が必要なので先に pool を作る
        pool = [(b["melspec"], b["target"]) for b in batch]
        out = []
        for b in batch:
            out.append(augmentor(b, pool))
        # 既存 default_collate で tensor 化
...
@hydra.main(config_path="conf", config_name="train_aug", version_base="1.1")
